...
%launch profile of the full scale rocket
clear
clc
...
load full_scale_data.txt

alpha_deg=2; %incoming angle (degrees); angle > 6.022; (38.6)
area=1200; %total area of one side of a fin
geo_center=0.08; %distance from the long end of the fin to the geometric
    %center of the area (m)

alpha_rad=alpha_deg*(PI/180); %calculated incoming angle (radians)
I=0.317961677; %entire rocket's axial moment of inertia (kg*m^2)
OD=0.15672; %outer diameter of lower stage of rocket
OD_upper=0.10414; %outer diamter of upper stage of rocket
time=full_scale_data(:,1); %time (s)
velocity=full_scale_data(:,2); %velocity (m/s)
altitude=full_scale_data(:,3); %altitude (m)
T=full_scale_data(:,4); %air temperature (K)
P=full_scale_data(:,5); %atmospheric pressure (Pa) or (N/m^2) or (kg/m*s^2)
rho=full_scale_data(:,6); %air density (kg/m^3)
M=full_scale_data(:,7); %velocity (mach #) [unitless value]
...
hold on
%if alpha_deg > 6.022
%    plot(full_scale_data(320:12000,1),Cl(320:12000),'b-')
%    legend('Implicit Model','Simplified Model','Mach Velocity')
%end
plot(altitude(320:12000,1),Cl_simp(320:12000),'r-')
plot(altitude(:),M(:),'m:')
xlabel('Rocket Height (m)')
ylabel('Coefficient of Lift/ Mach Number')
title('Coefficient of Lift at Supersonic Rocket Velocity with Full Rocket')
legend('Simplified Model','Mach Velocity')
hold off
...
%%%%% Add subsonic and transonic region to very beginning of plot