...
% Second rocket flights data: Base=11.25cm Height=7cm Length=4cm Moment
% of Inertia=0.0176kg*m^2 Diameter=4.75cm Mass=0.71kg
...
tab2=uitab('Parent',tgroup,'Title','Fin Flutter');
tab3=uitab('Parent',tgroup,'Title','Nutation Angle');
tab4=uitab('Parent',tgroup,'Title','Pitch Roll Coupling');
...
% legend(ax1_10,'Primary Forces','Secondary  Forces','Gravitational Forces','Reaction Forces and Moments');
axis off

plot(ax1_1,time,ohmega/(2*pi),'b')
xlim(ax1_1,[0, time(length(time))*1.1]);
ylim(ax1_1,[0, (max(ohmega)/(2*pi))*1.1]);

% plot(ax1_2,time,time,'b')
% xlim(ax1_2,[0, time(length(time))*1.1]);
% ylim(ax1_2,[min(velocityt)-0.5,max(velocityt)+0.5]);
% 
...
plot(ax1_5,rocket_flight(:,1),rocket_flight(:,2),'b',rocket_flight(:,1),rocket_flight(:,3),'k')
xlim(ax1_5,[0, rocket_flight(length(rocket_flight))*1.1]);
ylim(ax1_5,[min(rocket_flight(:,3))*1.1,max(rocket_flight(:,2))*1.1]);

title(ax1_1,'Rotation Rate');
title(ax1_2,'Fin Flutter');
title(ax1_3,'Nutation Angle');
title(ax1_4,'Pitch Roll Coupling');
title(ax1_5,'Rocket Flight Path');

ylabel(ax1_1,'Rotation Rate (Hz)');
ylabel(ax1_2,'Flutter Frequency (??)');
ylabel(ax1_3,'Nutation Angle (degrees)');
ylabel(ax1_4,'Pitch Angle (degrees)');
ylabel(ax1_5,'Variable (????)');
...
ohmega(length(ohmega))/(2*pi) %final value of spin in Hz at apogee
